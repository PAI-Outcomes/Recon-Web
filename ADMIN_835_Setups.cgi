...
sub processPharmacyData {
  ...
  my $sql = "SELECT R_TPP_PRI, R_TPP, max(R_BPR16_Date) as Latest_Remit, DATEDIFF(max(R_BPR16_Date), CURDATE()) as Date_Diff
               FROM (SELECT 'Archive' as DB, R_TPP_PRI, R_TPP, R_BPR16_Date
                       FROM $DBNAME.835remitstb_archive 
                      WHERE pharmacy_id = $Pharmacy_ID
                      UNION ALL 
                     SELECT 'Production' as DB, R_TPP_PRI, R_TPP, R_BPR16_Date 
                       FROM $DBNAME.835remitstb 
                      WHERE pharmacy_id = $Pharmacy_ID
                    ) all_remits
           GROUP BY R_TPP_PRI";
...
sub processTPPData {
  ...
  ### Gather REMIT information ### ------------------------------------------------------
  my $sql = "SELECT R_TPP_PRI, R_TPP, pharmacy_id, max(max_R_BPR16_Date) as Latest_Remit, DATEDIFF(max(max_R_BPR16_Date), CURDATE()) as Date_Diff
               FROM (SELECT 'Archive' as DB, R_TPP_PRI, R_TPP, pharmacy_id, max(R_BPR16_Date) as max_R_BPR16_Date 
                       FROM $DBNAME.835remitstb_archive 
                      WHERE R_TPP_PRI=$TPP_ID
                         && R_TS3_NCPDP NOT IN (1111111,2222222,3333333,4444444,5555555)
                   GROUP BY pharmacy_id
                     UNION ALL 
                     SELECT 'Production' as DB, R_TPP_PRI, R_TPP, pharmacy_id, max(R_BPR16_Date) as max_R_BPR16_Date
                       FROM $DBNAME.835remitstb 
                      WHERE R_TPP_PRI=$TPP_ID
                         && R_TS3_NCPDP NOT IN (1111111,2222222,3333333,4444444,5555555)
                   GROUP BY pharmacy_id
