...
if ( $testing ) {
  ...
  ########################################
    ...
    $DB1 = "RNDBNAME";
    $DB2 = "PADBNAME";
    $JJJ = $DBNAMES{"RNDBNAME"};
    print "JJJ: $JJJ<br>\n";
    print qq#B4: $DB1- $DBNAMES{"$DB1"}.$DBTABN{"$DB1"}<br>\n#;
    print qq#B4: $DB2- $DBNAMES{"$DB2"}.$DBTABN{"$DB2"}<br>\n#;
  
    $WHICHDB = "Testing";		# Valid Values: "Testing" or "Webinar"
    &set_Webinar_or_Testing_DBNames;
  
    $HHH = $DBNAMES{"RNDBNAME"};
    print "HHH: $HHH<br>\n";
    print qq#AF: $DB1- $DBNAMES{"$DB1"}.$DBTABN{"$DB1"}<br>\n#;
    print qq#AF: $DB2- $DBNAMES{"$DB2"}.$DBTABN{"$DB2"}<br>\n#;
...
sub GatherDataPCWNR {
  ...
  $sql  = "SELECT Pharmacy_ID, NCPDP, DateAdded, BIN, ThirdParty, PaymentType, CheckNumber,
                  CheckAmount, CheckDate, CheckReceivedDate, Notes
             FROM $DBNAME.$TABLE ";
  if ($ENV !~ /DEV/i ) {
    $sql .= "WHERE 1=1
  ...
  $sthx->finish;
  foreach $key (sort keys %NCPDPs) {
    $useNCPDP = sprintf("%07d", $NCPDPs{$key});
    $URLREMIT = "http://dev.recon-rx.com/tools/datatracking.php?ncpdp=$useNCPDP";
    $View1stRemitLines{$key} = qq#<a href="$URLREMIT" target=_blank">View</a>#;
  }
}
...
# sort by NCPDP, TPP, then Check Amounts
  ...
  $CURNCPDP = 0;
  foreach $Okey (sort {
      $Pharmacy_Names{$NCPDPs{$a}} cmp $Pharmacy_Names{$NCPDPs{$b}} ||
      $NCPDPs{$a} <=> $NCPDPs{$b} ||
      $TPPs{$a} cmp $TPPs{$b} ||
    ...
    $NCPDP         = $NCPDPs{$Okey};
    $Pharmacy_Name = $Pharmacy_Names{$NCPDPs{$Okey}};
...
#      print "<hr>array: $array, val: $val<br>\n";
       ...
       $STYLE = qq#style="font-size: 10px;"#;
       print "<td $ALIGN $STYLE>$val</td>";
       $WHITE .= "$val##";
    }
    print "</tr>\n";
...
# print qq#<INPUT style="background-color:\#FF0; padding:5px; margin:5px" TYPE="Submit" NAME="Submit" VALUE="Submit">\n#;
  ...
  print qq#<br /><div style="float: right;"><button style="padding:5px; margin:5px" id="submit_form" NAME="Submit">Save Changes</button></div>\n#;
...
# print qq#<tr><th colspan=$colspan><hr size=4 noshade color="black"></th></tr>\n#;
  ...
  foreach $array (@arrays) {
    ...
    $arrayname =~ s/PharmName/Pharmacy Name/gi;
#   $arrayname =~ s/PharmName/Name/gi;
    $arrayname =~ s/PaymentType/Pmt<br>Type/gi;
    $arrayname =~ s/DateAdded/Added/gi;
    $arrayname =~ s/View1stRemitLine/1st Remit/gi;
