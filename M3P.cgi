...
if ($PH_ID) {
  ...
  if($datacheck) {
    ...
    print qq#<h1>Sorry.  No Data Is Available At This Time</h1>\n
    #;
  }
 ## &build_line_report;
}
...
sub get_cats {
  my $sql = " 
              SELECT concat(Year(CURRENT_DATE - INTERVAL 12 MONTH),lpad(MONTH(CURRENT_DATE - INTERVAL 12 MONTH),2,0))
               UNION
              SELECT concat(Year(CURRENT_DATE - INTERVAL 11 MONTH),lpad(MONTH(CURRENT_DATE - INTERVAL 11 MONTH),2,0))
               UNION
	      SELECT concat(Year(CURRENT_DATE - INTERVAL 10 MONTH),lpad(MONTH(CURRENT_DATE - INTERVAL 10 MONTH),2,0))
	       UNION
	      SELECT concat(Year(CURRENT_DATE - INTERVAL 9 MONTH),lpad(MONTH(CURRENT_DATE - INTERVAL 9 MONTH),2,0))
	       UNION
	      SELECT concat(Year(CURRENT_DATE - INTERVAL 8 MONTH),lpad(MONTH(CURRENT_DATE - INTERVAL 8 MONTH),2,0))
	       UNION
	      SELECT concat(Year(CURRENT_DATE - INTERVAL 7 MONTH),lpad(MONTH(CURRENT_DATE - INTERVAL 7 MONTH),2,0))
               UNION
              SELECT concat(Year(CURRENT_DATE - INTERVAL 6 MONTH),lpad(MONTH(CURRENT_DATE - INTERVAL 6 MONTH),2,0))
               UNION
	      ...
	      SELECT concat(Year(CURRENT_DATE - INTERVAL 1 MONTH),lpad(MONTH(CURRENT_DATE - INTERVAL 1 MONTH),2,0))
	       UNION
	      SELECT concat(Year(CURRENT_DATE),lpad(MONTH(CURRENT_DATE),2,0))
           ";
...
my $mnth;
  ...
  foreach $mnth (@months) {
    foreach $payer (@payers) {
      $amt = 0;
      my $sql = " SELECT yearmo, third_party_payer_name, sum(Total_M3P)  
                    FROM $DBNAME.m3p_monthly  a
                    JOIN officedb.third_party_payers b on a.tpp_id = b.third_party_payer_id
                   WHERE Pharmacy_ID IN ($PH_ID) 
                     AND yearmo = $mnth 
  ...
  print qq#
    ...
    \$(function () {
	  ...
	  style: { color: '\#133562', fontWeight: 'bold', fontSize: '18px', fontFamily: 'Trebuchet MS' },
          text: 'Total M3P (Rolling Year Review)'
        },
        subtitle: {
	  style: { color: '\#133562', fontSize: '16px', fontFamily: 'Trebuchet MS' },
        ...
        xAxis: {
          categories: [ @cat ],
          labels: {
            format: '<h3 style= "padding:0px; margin:0px;">{value}</h3></div>',
            useHTML: true
          },
        },
        yAxis: {
