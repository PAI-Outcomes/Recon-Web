  ...
  ## print qq#<script type="text/javascript" charset="utf-8" src="https://code.jquery.com/jquery-3.5.1.js"></script> \n#;
   ...
   print qq#<script type="text/javascript" charset="utf-8">
           ...
           function sort_records(col) {
              var iForm = document.getElementById(selectForm)
              element   = document.getElementById('sortby');
...
#   print "TYPE: $CSR_Types{$CSR_Reverse_ID_Lookup{$USER}}, USER: $CSR_Reverse_ID_Lookup{$USER}, $USER<br>";
   ...
   if ( $CSR_Types{$CSR_Reverse_ID_Lookup{$USER}} =~ /Admin/i ) {
     ...
     print qq#<input type="hidden" id="sortby" name="sortby" value="$sortby">#;
     ...
     if ( $all_msg ) {
       ...
       $and .= " && a.id in ( 
         SELECT a.id
           ...
           JOIN ReconRxdb.communication_dtl b ON (a.id = b.comm_id) JOIN officedb.pharmacy c ON (a.Pharmacy_ID = c.Pharmacy_ID) 
          WHERE (a.status IN ($c_status) AND a.dte_upd >= CURRENT_TIMESTAMP - INTERVAL $months_ago MONTH)
           $and 
             && (a.user_id NOT IN ($tmp_user)  || b.user_id NOT IN ($tmp_user))
          )";
     }
     else {
   ...
   $sql = "SELECT a.id, a.user_id, a.name, a.subject, a.dte_add, a.dte_upd,
            ...
            WHERE ((a.status IN ('V') AND a.dte_upd >= CURRENT_TIMESTAMP - INTERVAL 1 MONTH) || (a.status in ('N')))
              $and
          ORDER BY $order $descasc";

   ($sqlout = $sql) =~ s/\n/<br>\n/g;

  ## print "$sql" if($USER == 69);
 #  print "$sql" if($USER == 74);
