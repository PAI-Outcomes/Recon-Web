...
sub check_Arete {
  foreach $Pharmacy_ID (keys %Pharmacies) {
    $Arete = 1 if ($Pharmacy_Arete{$Pharmacy_ID}); 
  }
}
sub displayWebPage {
   ...
   print "<thead>\n";
   print "<tr><th>$nbsp</th><th width='380px'>Pharmacy Name</th><th>NCPDP</th><th>NPI</th><th>Phone</th><th>Address</th><th>ID</th></tr>\n";
   print "</thead>\n";
   print "<tbody>\n";

   if ($Aggregated) {
      foreach $Pharmacy_ID (keys %Pharmacies) {
        push (@pids,$Pharmacy_ID);
      }
      $pidstr = join(",", @pids);
      print "<tr>";
      $URLH = "MyReconRx.cgi";
      print qq#<FORM ACTION="$URLH" METHOD="POST">\n#;
      print qq#<INPUT TYPE="hidden" NAME="Agg_String" VALUE="$pidstr">\n#;
      print qq#<INPUT TYPE="hidden" NAME="PH_ID" VALUE="Aggregated">\n#;
      print qq#<td>#;
      print qq#<INPUT TYPE="Submit" NAME="Submit" VALUE="Select">\n#;
      print qq#</FORM>\n#;
      print qq#</td>#;
      print "<td width=150>Aggregated</td>\n";
      print "<td>ALL</td>\n";
      print "<td></td>\n";
      print "<td></td>\n";
      print "<td></td>\n";
...
sub process_notifications {
   ...
   foreach $dirname (@dirnames) {
      if ( $ENV =~ /DEV/i ) {
         ...
         $dskpath = "\\\\webprod\\WWW\\www.recon-rx.com\\Webshare\\$dirname";
      }
