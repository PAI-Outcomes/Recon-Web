...
sub displayWebPage {
  ...
  $outdir    = qq#\\\\$WBSERVER\\Webshare (ReconRx)\\End_Of_Month_NRID${testing}#;
  ...
  @files = grep(/\_${inNCPDP}_/,readdir(DIR));
  $file = @files;
  ...
  foreach $key (sort { $b <=> $a } keys %dofiles) {
     ...
     $webpath = "https://${ENV}.Recon-Rx.com/Webshare/End_Of_Month_NRID${testing}/$fname";
