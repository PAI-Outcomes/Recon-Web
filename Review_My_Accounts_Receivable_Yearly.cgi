...
$ntitle = "Yearly Accounts Receivable Reports for $inNCPDP";
...
# Look for End_of_Year file
  if ( $ENV =~ /Dev/i ) {
     ...
     $outdir    = qq#\\\\$WBSERVER\\Webshare (ReconRx)\\End_of_Fiscal_Year${testing}$Agg#;
  }
  ...
  foreach $key (sort { $b <=> $a } keys %dofiles) {
     ...
     $webpath = "https://${ENV}.Recon-Rx.com/Webshare/End_of_Fiscal_Year${testing}$Agg2/$fname";
