...
$ntitle = "Monthly Claims Summary Reports for $inNCPDP";
...
# Look for End_of_Month file
  if ( $ENV =~ /Dev/i ) {
     $outdir    = qq#D:\\WWW\\www.recon-rx.com\\WebShare\\End_of_Month_Reconciled_Claims$Agg#;
  } else {
     $outdir    = qq#\\\\$WBSERVER\\Webshare (ReconRx)\\End_of_Month_Reconciled_Claims${testing}$Agg#;
  }
  ...
  @files = grep(/${inNCPDP}_${ID}_/,@files);
  $file = @files;
  ...
  foreach $key (sort { $b <=> $a } keys %dofiles) {
     ...
     $webpath = "https://${ENV}.Recon-Rx.com/Webshare/End_of_Month_Reconciled_Claims${testing}$Agg2/$fname";
