...
$ntitle = "Quarterly Claims Summary Reports for $inNCPDP";
...
# Look for End_of_Month file
  if ( $ENV =~ /Dev/i ) {
     $outdir    = qq#D:\\WWW\\www.recon-rx.com\\WebShare\\QTR_Reconciled_Claims$Agg_Reconciled_Claims$Agg#;
  } else {
     $outdir    = qq#\\\\$WBSERVER\\Webshare (ReconRx)\\End_of_Quarter_Reconciled_Claims${testing}$Agg#;
  }
  ...
  foreach $key (sort { $b <=> $a } keys %dofiles) {
     ...
     $webpath = "https://${ENV}.Recon-Rx.com/Webshare/End_of_Quarter_Reconciled_Claims${testing}$Agg2/$fname";
