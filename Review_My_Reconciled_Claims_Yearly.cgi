...
$ntitle = "Yearly Claims Summary Reports for $inNCPDP";
...
# Look for End_of_Year file
  if ( $ENV =~ /Dev/i ) {
     $outdir    = qq#D:\\WWW\\www.recon-rx.com\\WebShare\\End_of_Fiscal_Year_Reconciled_Claims$Agg#;
  } else {
     $outdir    = qq#\\\\$WBSERVER\\Webshare (ReconRx)\\End_of_Fiscal_Year_Reconciled_Claims${testing}$Agg#;
  }
  ...
  foreach $key (sort { $b <=> $a } keys %dofiles) {
     ...
     $webpath = "https://${ENV}.Recon-Rx.com/Webshare/End_of_Fiscal_Year_Reconciled_Claims${testing}$Agg2/$fname";
