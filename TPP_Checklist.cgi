...
#  # selected a Date from previous page
   ...
   if ( $numofrows > 0 ) {
     while (my @row = $stb->fetchrow_array()) {
       ...
       if ( $R_REF02_Value !~ /^\s*$/ ) {
          ##Took this out.  You can get 835 remit in detailed search
     ...
     foreach $plbs (sort keys %missingplb) {
       $type = 'Adjustments';
       ($pcheck,$pcheckdate,$cid,$payid)  =  split("##", $plbs);
     
       if ($TPP_Names{$payid}) {
         $type .= "($TPP_Names{$payid})";
       }
     ...
     foreach $Check_ID (sort keys %ListAtBottom) {
        if ($firstone) {
           ###20240917  I took this out because it was broken and you can get the 835 from the detailed search##
  ...
  while (my @row = $stb->fetchrow_array()) {
     ...
     ####GETS TOTAL BY PAYER#####
       if ( $Lookup_TPP_Display_on_Remit_TPP_IDs{$key} ) {
         $lkey = "$check_id##$Lookup_TPP_Display_on_Remit_TPP_IDs{$key}";
     ...
     if($plb && !$reflist{$check}{$plb}){
         $missingplb{"$check##$checkdate##$lkey"} += $plbamt;
     }
  }
}
