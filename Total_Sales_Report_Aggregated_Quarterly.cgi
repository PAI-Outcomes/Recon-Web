...
$ntitle = "Total Sales Report Aggregated Quarterly";
...
sub displayWebPage {
  ...
  if ( $ENV =~ /Dev/i ) {
     $outdir    = qq#D:\\WWW\\www.recon-rx.com\\WebShare\\End_of_Quarter_Total_Sales_Report_Aggregated#;
  } else {
     $outdir    = qq#\\\\$WBSERVER\\Webshare (ReconRx)\\End_of_Quarter_Total_Sales_Report_Aggregated#;
  }
 ...
 ##   $pcs[6] =~ s/\.xlsx//gi;
  ...
  foreach $key (sort { $b <=> $a } keys %dofiles) {
    ...
    $webpath = "https://${ENV}.Recon-Rx.com/Webshare/End_of_Quarter_Total_Sales_Report_Aggregated/$fname";
